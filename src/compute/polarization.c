...
static const char* SHADER_PATH = "shaders/polarization.comp.spv";

bool polarization_create(vk_context* ctx, polarization_renderer* renderer, u32 width, u32 height) {
    memset(renderer, 0, sizeof(polarization_renderer));
    ...
    push.ranges[0] = (VkPushConstantRange){
        ...
        .offset = 0,
        .size = sizeof(polarization_params),
    };
    push.range_count = 1;
...
void polarization_render(vk_context* ctx, polarization_renderer* renderer, VkCommandBuffer cmd, u32 frame_index) {
    vk_cmd_begin_label(ctx, cmd, "Polarization", (float[4]){0.2f, 1.0f, 0.6f, 1.0f});
