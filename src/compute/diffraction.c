...
static const char* SHADER_PATH = "shaders/diffraction.comp.spv";

bool diffraction_create(vk_context* ctx, diffraction_renderer* renderer, u32 width, u32 height) {
    memset(renderer, 0, sizeof(diffraction_renderer));
    ...
    push.ranges[0] = (VkPushConstantRange){
        ...
        .offset = 0,
        .size = sizeof(diffraction_params),
    };
    push.range_count = 1;
...
void diffraction_render(vk_context* ctx, diffraction_renderer* renderer, VkCommandBuffer cmd, u32 frame_index) {
    vk_cmd_begin_label(ctx, cmd, "Diffraction", (float[4]){0.6f, 0.2f, 1.0f, 1.0f});
